      ...
      <div class="bg-white rounded-xl shadow-lg p-6">
        <div class="flex flex-col md:flex-row gap-4">
          <div class="flex-1">
            ...
            <select [(ngModel)]="statusFilter" (change)="filterByStatus()" class="input-field w-48">
...
export class ReviewClaimsComponent implements OnInit {
  ...
  // Computed property to auto-filter when the signal updates
  filteredClaims = computed(() => {
    const query = this.searchQuery.toLowerCase();
    const sourceClaims = this.claims();
  ...
  loadClaims(status: string) {
    this.claimService.getAdminClaims(status).subscribe({
      next: (data) => {
        this.claims.set(data.map(c => ({
          ...c,
          reviewDecision: null,
          approvedAmount: null,
          decisionReason: ''
  ...
  filterByStatus() {
    this.loadClaims(this.statusFilter);
  }

  // filterClaims is now replaced by the filteredClaims computed!
  filterClaims() {
    // Keep it here as a no-op just in case the template runs it on (input)
  }
